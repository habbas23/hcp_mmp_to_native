rule convert_to_gifti:
    ...
    output: 'results/sub-{subject}/{hemi}.{surfname}.surf.gii'
...
rule convert_to_nifti:
    ...
    output: 'results/sub-{subject}/{volname}.nii.gz'
    singularity: config['singularity_freesurfer']
...
rule get_tkr2scanner:
    input: 
        t1 = 'results/sub-{subject}/T1.nii.gz'
    output:
        tkr2scanner = 'results/sub-{subject}/tkr2scanner.xfm'
    singularity: config['singularity_freesurfer']
    shell: 'mri_info {input.t1} --tkr2scanner > {output.tkr2scanner}'
     
rule apply_surf_tkr2scanner:
    input: 
        surf = 'results/sub-{subject}/{hemi}.{surfname}.surf.gii',
        tkr2scanner = 'results/sub-{subject}/tkr2scanner.xfm'
    output: 
        surf = 'results/sub-{subject}/{hemi}-scanner.{surfname}.surf.gii'
    threads: 8
...
rule gen_midthickness:
    input:
        white = 'results/sub-{subject}/{hemi}.white.surf.gii',
        pial = 'results/sub-{subject}/{hemi}.pial.surf.gii'
    output: 
        midthickness = 'results/sub-{subject}/{hemi}.midthickness.surf.gii'
    singularity: config['singularity_connectome_workbench']
    threads: 8
...
rule resample_subj_to_fsaverage_sphere:
    input: 
        surf = 'results/sub-{subject}/{hemi}.midthickness.surf.gii',
        current_sphere = 'results/sub-{subject}/{hemi}.sphere.reg.surf.gii',
    ...
    params:
        method = 'BARYCENTRIC'
    output:
        surf = 'results/sub-{subject}/{hemi}.midthickness.32k_fs_LR.surf.gii'
    singularity: config['singularity_connectome_workbench']
    threads: 8
...
rule resample_labels_to_subj_sphere:
    input:
        ...
        current_sphere = lambda wildcards: 'resources/standard_mesh_atlases/resample_fsaverage/'
                                            'fs_LR-deformed_to-fsaverage.{H}.sphere.32k_fs_LR.surf.gii'.format(
                                                H = hemi_to_H[wildcards.hemi]),
        new_sphere = 'results/sub-{subject}/{hemi}.sphere.reg.surf.gii',
        current_surf = 'results/sub-{subject}/{hemi}.midthickness.32k_fs_LR.surf.gii',
        new_surf = 'results/sub-{subject}/{hemi}.midthickness.surf.gii'
    params:
        method = 'ADAP_BARY_AREA'
    output:
        label = 'results/sub-{subject}/{hemi}.native.hcp-mmp.label.gii'
    singularity: config['singularity_connectome_workbench']
    threads: 8
    shell: 
...
rule map_labels_to_volume_ribbon:
    input: 
        label = 'results/sub-{subject}/{hemi}.native.hcp-mmp.label.gii',
        surf = 'results/sub-{subject}/{hemi}-scanner.midthickness.surf.gii',
        vol_ref = 'results/sub-{subject}/T1.nii.gz',
        white_surf = 'results/sub-{subject}/{hemi}-scanner.white.surf.gii',
        pial_surf = 'results/sub-{subject}/{hemi}-scanner.pial.surf.gii',
    output:
        label_vol = 'results/sub-{subject}/{hemi}.native.hcp-mmp.nii.gz',
    singularity: config['singularity_connectome_workbench']
    threads: 8
    shell:
...
rule map_labels_to_volume_wmboundary:
    input: 
        label = 'results/sub-{subject}/{hemi}.native.hcp-mmp.label.gii',
        surf = 'results/sub-{subject}/{hemi}.white.surf.gii',
        vol_ref = 'results/sub-{subject}/T1.nii.gz',
        white_surf = 'results/sub-{subject}/{hemi}.white.surf.gii',
        pial_surf = 'results/sub-{subject}/{hemi}.pial.surf.gii',
    params:
        nearest_vertex = '{wmbdy}' 
    output:
        label_vol = 'results/sub-{subject}/{hemi}.native-wmbound{wmbdy}.hcp-mmp.nii.gz',
    singularity: config['singularity_connectome_workbench']
    threads: 8
    shell:
        'OMP_NUM_THREADS={threads} wb_command -label-to-volume-mapping {input.label} {input.surf} {input.vol_ref} {output.label_vol}'
